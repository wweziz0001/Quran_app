...
async function main() {
  ...
  // 4. Update Surah slugs
  console.log('📝 Updating Surah slugs...');
  const surahSlugsByNumber: Record<number, string> = {
    1: 'al-fatiha',
    2: 'al-baqarah',
    3: 'ali-imran',
    4: 'an-nisa',
  ...
  let updatedSlugs = 0;
  for (const [numStr, slug] of Object.entries(surahSlugsByNumber)) {
    const number = parseInt(numStr);
    const result = await prisma.surah.updateMany({
      where: { number },
      data: { slug },
    });
    if (result.count > 0) updatedSlugs++;
  }
  console.log(`✅ Updated ${updatedSlugs} Surah slugs`);
